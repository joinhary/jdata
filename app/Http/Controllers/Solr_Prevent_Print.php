...
class Solr_Prevent_Print extends Controller
{
    protected $client;
    ...
    public function XuLyTimKiem_duong_su2($data, $option, $request)
    {
        if (preg_match_all('/\"([^\"]*?)\"/', $data, $matches)) {
            ...
            $page = $request->page ?? 1;
            $numberInPage = 20;
            $query->setQuery($string_query)->setQueryDefaultOperator('AND')->setStart(0)->setRows(1000);
        ...
        if (preg_match_all('/\"([^\"]*?)\"/', $data, $matches)) {
            ...
            foreach ($search2 as $key => $val) {
                if ($val && $option == 1) {
                    ...
                    $val = str_replace('-'," ", $val);
                    $sub_string_query =  ' AND merge_content:' . '(' . '"' . $search1 . '"' . '*' . $val . '*' . ')';                    $string_query .= $sub_string_query;
                } else {
                    continue;
                }
            }
    ...
    public function search(Request $request)
    {
        ...
        if ($option == []) {
            ...
            $query = $this->client->createSelect();
            $query2 = $this->client->createSelect();
            $query->setQuery('ngan_chan:(3)')->setQueryDefaultOperator('OR')->setStart(0)->setRows(20);
            $resultset = $this->client->select($query);
            $data = $this->fetchData($resultset);
            $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
            $count = $resultset->getNumFound();
        ...
        if ($request->duong_su == null && $request->tai_san == null && $request->tat_ca == null) {
            ...
            $query = $this->client->createSelect();
            $query2 = $this->client->createSelect();
            $query->setQuery('ngan_chan:(3)')->setQueryDefaultOperator('OR')->setStart(0)->setRows(1000);
            $resultset = $this->client->select($query);
            $data = $this->fetchData($resultset);
            $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
            $count = $resultset->getNumFound();
        ...
        if ($request->duong_su != null && $request->tai_san != null && $request->tat_ca == null) {
            ...
            if (preg_match('/[\x80-\xFF]/', $request->duong_su) || preg_match('/[\x80-\xFF]/', $request->tai_san)) {
                ...
                if (strpos($request->duong_su, '"') !== false && strpos($request->tai_san, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su:' . '(' . $request->duong_su . ') AND texte:' . '(' . $request->tai_san . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
                ...
                if (strpos($request->duong_su, '"') !== false && strpos($request->tai_san, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su_en:' . '(' . $request->duong_su . ') AND texte_en:' . '(' . $request->tai_san . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
        ...
        if ($request->duong_su != null) {
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su_en:' . '(' . $search . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
        ...
        elseif ($request->tai_san != null) {
            ...
            if (preg_match('/[\x80-\xFF]/', $search)) {
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('texte:' . '(' . $search . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('texte_en:' . '(' . $search . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
            ...
            if (preg_match('/[\x80-\xFF]/', $search)) {
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $numberInPage = 20;
                    $query = $this->client->createSelect();
                    $query->setQuery('texte:' . '(' . $search . ') duong_su:' . '(' . $search . ') AND ngan_chan:(3) ')->setStart(0)->setRows(1000);
                    $query2 = $this->client->createSelect();
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $numberInPage = 20;
                    $query = $this->client->createSelect();
                    $query->setQuery('texte_en:' . '(' . $search . ') duong_su_en:' . '(' . $search . ') AND ngan_chan:(3)')->setStart(0)->setRows(1000);
                    $query2 = $this->client->createSelect();
                    $query->setQueryDefaultOperator('OR')->setStart(0)->setRows(1000);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
