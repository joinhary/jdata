...
//            ->orderBy("created_at")
        ...
        if ($request->tk_noidung) {
            if ($request->loaitk == 'label') {
                $where[] = ['nhan', 'LIKE', '%' . $request->tk_noidung . '%'];
            }
            if ($request->loaitk == 'code') {
                $where[] = ['id', '=', $request->tk_noidung];
            }
        }
        $ds_hopdong = HopDongModel::select('id', 'nhan','ngayky')
            ->where($where)->paginate(20);
        ;
        if(Sentinel::check())
return redirect('admin/suutra/index');
        else
            return redirect('admin/signin')->with('error', 'You must be logged in!');
    }

    public function gethd(Request $request)
    {
//        return
    }
