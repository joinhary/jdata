...
use App\Http\Controllers\Solr_Office_Print;
...
class SolariumController_vp extends Controller
{
    protected $client;
    ...
    public function __construct(\Solarium\Client $client)
    {
        ...
        $query2->setQuery("ngan_chan: 3")->setQueryDefaultOperator('OR')->setStart(0)->setRows(20);
    ...
    public function XuLyTimKiem_tat_ca2($data, $option, $request)
    {
        ...
        if (preg_match_all('/\"([^\"]*?)\"/', $data, $matches)) {
            ...
            $page = $request->page ?? 1;
            $numberInPage = 20;
            $query->setQuery($string_query)->setQueryDefaultOperator('OR')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
            $resultset = $this->client->select($query);
            $data = $this->fetchData($resultset);
            $count = $resultset->getNumFound();
            //oday
    ...
    public function XuLyTimKiem_2_o($texte, $duongsu, $option, $request)
    {
        ...
        if (preg_match_all('/\"([^\"]*?)\"/', $duongsu, $matches1) && preg_match_all('/\"([^\"]*?)\"/', $texte, $matches2)) {
            ...
            $page = $request->page ?? 1;
            $numberInPage = 20;
            $query->addSort('st_id', $query::SORT_DESC)->setQuery($string_query)->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
    ...
    public function search(Request $request)
    {
            ...
            //Thông báo chung-----------------------------------------------
            if ($request->all() !== []) {
                if ($request->duong_su3 == null && $request->tai_san3 == null && $request->tat_ca3 == null) {
                    $Tbc = [];
                } else {
        ...
        $query5->setQuery('sync_code:' . '(' . $cn_ndd . ')')->setQueryDefaultOperator('OR')->setStart(0)->setRows(20);
        ...
        if ($option == []) {
            ...
            $query = $this->client->createSelect();
            $query2 = $this->client->createSelect();
            $query->setQuery('sync_code:' . '(' . $cn_ndd . ')')->setQueryDefaultOperator('OR')->addSort('st_id', $query::SORT_DESC)->setStart(0)->setRows(20);
            $resultset = $this->client->select($query);
            $data = $this->fetchData($resultset);
            $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
            $count = $resultset->getNumFound();
        ...
        if ($request->duong_su3 == null && $request->tai_san3 == null && $request->tat_ca3 == null && $request->so_hd3 == null) {
            // dd(1);
            $page = $request->page ?? 1;
$numberInPage = 20;

$query = $this->client->createSelect();
$query->setQuery('sync_code:"' . $cn_ndd . '"')
      ->setQueryDefaultOperator('AND')
      ->addSort('st_id', $query::SORT_DESC)
      ->setStart(($page - 1) * $numberInPage)
      ->setRows($numberInPage);

$resultset = $this->client->select($query);

$dataRaw = $this->fetchData($resultset);
$count   = $resultset->getNumFound();

$optionPaginate = [
    'path'  => route('searchSolr_vp'),
    'query' => $request->query(),
];

$data = $this->paginate($dataRaw, $numberInPage, $page, $optionPaginate, $count);
// dd($data[0]);
            // $data = $this->paginate($data, $numberInPage, $page, $optionPaginate, $count);
...
// dd($option);
        ...
        if ($request->duong_su3 != null && $request->tai_san3 != null && $request->tat_ca3 == null) {
            $duong_su3 = $this->vn_to_str($request->duong_su3);
            $tai_san3 = $this->vn_to_str($request->tai_san3);
            $search = $request->duong_su3;
            $page = $request->page ?? 1;
            $numberInPage = 20;
            if (preg_match('/[\x80-\xFF]/', $request->duong_su3) || preg_match('/[\x80-\xFF]/', $request->tai_san3)) {
                //Chứa dấu " "
                if (strpos($request->duong_su3, '"') !== false && strpos($request->tai_san3, '"') !== false) {
                    // cả 2 đều có ""
                    $data = $this->XuLyTimKiem_2_o($request->tai_san3, $request->duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                ...
                } elseif (strpos($request->duong_su3, '"') !== false && strpos($request->tai_san3, '"') === false) {
                    $data = $this->XuLyTimKiem_2_o_duong_su($request->tai_san3, $request->duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    $string_out = array_map(function ($item) {
                    ...
                    $data = $this->XuLyTimKiem_2_o_tai_san($request->tai_san3, $request->duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    $string_out = array_map(function ($item) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su:' . '(' . $request->duong_su3 . ') AND texte:' . '(' . $request->tai_san3 . ') AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
            ...
            } elseif (preg_match('/[\x80-\xFF]/', $duong_su3) && preg_match('/[\x80-\xFF]/', $tai_san3) == false) {
                if (strpos($duong_su3, '"') !== false && strpos($tai_san3, '"') !== false) {
                    // cả 2 đều có ""
                    dd('cc');
                    $data = $this->XuLyTimKiem_2_o_1_0($tai_san3, $duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    //add string_in to string_out
                    ...
                    $data = $data[0];
                    return view('search', compact('data', 'Tbc', 'option', 'type', 'count', 'search', 'code_cn', 'count_bs', 'count_ngan_chan', 'count_office', 'string_out', 'count_ngan_chan_result'));
                } elseif (strpos($duong_su3, '"') !== false && strpos($tai_san3, '"') === false) {
                    $data = $this->XuLyTimKiem_2_o_duong_su($tai_san3, $duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    //add string_in to string_out
                    ...
                    $data = $this->XuLyTimKiem_2_o_tai_san($tai_san3, $duong_su3, 1, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    //add string_in to string_out
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su:' . '(' . $duong_su3 . ') AND texte:' . '(' . $tai_san3 . ') AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                ...
                if (strpos($request->duong_su3, '"') !== false && strpos($request->tai_san3, '"') !== false) {
                    // cả 2 đều có ""
                    $data = $this->XuLyTimKiem_2_o($request->tai_san3, $request->duong_su3, 2, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    $string_out = array_map(function ($item) {
                    ...
                    $data = $this->XuLyTimKiem_2_o_duong_su($request->tai_san3, $request->duong_su3, 2, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    $string_out = array_map(function ($item) {
                    ...
                    $data = $this->XuLyTimKiem_2_o_tai_san($request->tai_san3, $request->duong_su3, 2, $request);
                    $string_in = $data[2] ?? '';
                    $string_in = str_replace('"', '', $string_in);
                    $string_out = $data[3] ?? [];
                    $string_out = array_map(function ($item) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su_en:' . '(' . $request->duong_su3 . ') AND texte_en:' . '(' . $request->tai_san3 . ') AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_nganchan'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
                    $data = $this->paginate($data, $numberInPage, $page, $optionPaginate, $count);
                    $string_out = explode(" ", $search . ' ' . $tai_san3);
                    $string_out = json_encode($string_out, JSON_UNESCAPED_UNICODE);
                    return view('search', compact('data', 'Tbc', 'option', 'type', 'count', 'search', 'code_cn', 'count_bs', 'count_ngan_chan', 'count_office', 'string_out', 'count_ngan_chan_result'));
                }
            }
        ...
        if ($request->duong_su3 != null) {
            ...
            if (preg_match('/[\x80-\xFF]/', $search)) {
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su:' . '(' . $search . ') AND sync_code:' . '(' . $cn_ndd . ')')->setQueryDefaultOperator('OR')->addSort('st_id', $query::SORT_DESC)->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query2 = $this->client->createSelect();
                    $query->setQuery('duong_su_en:' . '(' . $search . ')AND sync_code:' . '(' . $cn_ndd . ')')->setQueryDefaultOperator('AND')->addSort('st_id', $query::SORT_DESC)->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
        ...
        elseif ($request->tai_san3 != null) {
            $search = $request->tai_san3;
            //check is request vietnamese string ?
            if (preg_match('/[\x80-\xFF]/', $search)) {
                //Chứa dấu " "
                if (strpos($search, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('texte:' . '(' . $search . ')AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $query = $this->client->createSelect();
                    $query2 = $this->client->createSelect();
                    $query->setQuery('texte_en:' . '(' . $search . ')AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
        ...
        //Tim theo So_hd
        elseif ($request->so_hd3 != null) {
            
           $rawSearch = trim($request->so_hd3 ?? '');

// người dùng có nhập "..." hay không
$isPhraseSearch = preg_match('/^".+"$/', $rawSearch);
...
$search = $this->vn_to_str($search);
            if (preg_match('/[\x80-\xFF]/', $search)) {
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    $numberInPage = 20;
                    $query = $this->client->createSelect();
                    $query->setQuery('texte_en:' . '(' . $search . ') OR duong_su_en:' . '(' . $search . ') AND sync_code:' . '(' . $cn_ndd . ') ')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $query2 = $this->client->createSelect();
                    $query->setQueryDefaultOperator('OR')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
                    $resultset = $this->client->select($query);
                    $data = $this->fetchData($resultset);
                    $optionPaginate = ['path' => route('searchSolr_vp'), 'query' => $request->query()];
                    $count = $resultset->getNumFound();
            ...
            //Tìm không dấu
                ...
                if (strpos($search, '"') !== false) {
                    ...
                    // dd(1);
                    // return view('search', compact('data', 'Tbc', 'option', 'type', 'count', 'search', 'code_cn', 'count_bs', 'count_ngan_chan', 'count_office', 'string_out', 'count_ngan_chan_result'));
              $searchRaw = trim($request->tat_ca3 ?? '');


    // ===== 1. Chuẩn hoá =====
    $searchRaw = str_replace('-', ' ', $searchRaw);
    ...
    $vpcc_id = NhanVienModel::where('nv_id', $user_id)->value('nv_vanphong');
    $cn_ndd  = ChiNhanhModel::where('cn_id', (int)$vpcc_id)->value('code_cn');
    ...
    if (strpos($searchRaw, '"') !== false) {
        ...
        /**
         * Không ngoặc kép → dùng edismax
         * (ổn định, không lỗi precedence)
         */
        $query->setQuery($searchRaw);
        $query->addParam('defType', 'edismax');
        $query->addParam('qf', 'duong_su tai_san texte');
        $query->addParam('fq', 'sync_code:' . $cn_ndd);
    }

    // ===== 5. Sort + paging =====
    $query->addSort('st_id', $query::SORT_DESC)
          ->setStart(($page - 1) * $numberInPage)
          ->setRows($numberInPage);
    ...
    // ===== 8. Highlight keyword =====
    if (strpos($searchRaw, '"') !== false) {
        $string_out = [str_replace('"', '', $searchRaw)];
    } else {
        $string_out = explode(' ', $searchRaw);
    }

    $string_out = json_encode(array_filter($string_out), JSON_UNESCAPED_UNICODE);
...
$string_out = json_encode(
    array_values(array_filter(explode(' ', trim($search, '"')))),
...
return view(
    ...
    compact(
        ...
        'count_office',
        'string_out',
        'count_ngan_chan_result'
    )
);
//  $page = $request->page ?? 1;
                //         $numberInPage = 20;
                //         $query = $this->client->createSelect();
                //         $query->setQuery('merge_content:' . '(' . $search . ') OR so_hd:' . '(' . $search . ') AND sync_code:' . '(' . $cn_ndd . ')')->setStart(($page - 1) * $numberInPage)->setRows(($page) * $numberInPage);
    ...
    public function printSolr(Request $request)
    {
        ...
        $search = $request->all();
        $Solr_Office_Print = new Solr_Office_Print($this->client);
        //filter item not null
        $search = array_filter($search);
        $search = implode(' ', $search);
        $search = str_replace('"', '', $search);
        $data =   $data = $Solr_Office_Print->search($request);
    ...
    public function XuLyTrungHoSo(Request $request)
    {

       $result = $this->XuLyTimKiem_so_hd($request->code,2,$request);
       $total = $result[1];
      if($total > 0){
        return response()->json([
            'result' => false,
            'message' => 'LỖI: Đã tồn tại số hợp đồng này, vui lòng kiểm tra lại!'
        ], 200);
      }else{
        return response()->json([
            'result' => true,
