...
class SPPSolariumController extends Controller
{
    protected $client;
    ...
    public function fetchData($resultset)
    {
        ...
        foreach ($resultset as $document) {
            $data[] = [
                'duong_su' => $document->duong_su[0],
                'tai_san' => $document->texte[0],
                'ngay_nhap' =>  $document->ngay_nhap[0],
                'ngay_cc' => $document->ngay_cc[0],
                'so_hd' => $document->so_hd[0],
                'ten_hd' => $document->ten_hd[0],
                'ngay_chan' => $document->ngay_chan[0],
                'vp_master' => $document->vp_master[0],
                'ccv_master' => $document->ccv_master[0],
                'cancel_description' => $document->cancel_description[0],
                'contract_period' => $document->contract_period[0],
                'is_ngan_chan' => $document->ngan_chan[0],
                'sync_code' => $document->sync_code[0],
                'created_at' => $document->created_at[0],
                'updated_at' => $document->updated_at[0],
                'release_doc_number' => $document->release_doc_number[0],
                'release_doc_date' => $document->release_doc_date[0],
                'deleted_note' => $document->deleted_note[0],
                'picture' => $document->picture[0],
                'real_name' => $document->real_name[0],
                'release_file_path' => $document->release_file_path[0],
                'note' => $document->note[0],
                'ma_phan_biet' => $document->ma_phan_biet[0],
                'release_file_name' => $document->release_file_name[0],
                'undisputed_date' => $document->undisputed_date[0],
                'undisputed_note' => $document->undisputed_note[0],
            ];
        }
        $data = collect($data);
        return $data;
