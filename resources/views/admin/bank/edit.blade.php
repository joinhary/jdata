...
@section('content')
    <section class="content">
        <form method="POST" action="{{route('updateBank',['id'=>$bank->id])}}" class="form-create"
              enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        <label for="name">Tên chi nhánh: (<span class="text-danger qksao">*</span>)</label>
                        <input type="text" id="name" class="form-control" name="name"
                               value="{{$bank->name}}"
                               required autofocus TABINDEX="1">
                    </div>
                    <div class="row">
                        <label for="order_number">Mã số: </label>
                        <input type="text" id="order_number" class="form-control" name="order_number" value="{{$bank->order_number}}"
                               TABINDEX="2">
                    </div>
                    {{-- <div class="row">
                    ...
                    <br>
                    <div class="row">
                        <a href="{{route('indexBank')}}" type="cancel" class="btn btn-secondary qkbtn">Hủy</a>
