...
@section('content')
    <section class="content">
        <form method="POST" action="{{ route('admin.templates.tai-san.store') }}" class="form-create" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
                        <label for="nv_vanphong">Kiểu tài sản: (<span class="text-danger qksao">*</span>)</label>
                        <select id="slug" class="form-control select2" name="slug">
                            @foreach($kieuTaiSan as $kieu)
                                <option value="{{ $kieu->k_keywords }}">{{ $kieu->k_nhan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
