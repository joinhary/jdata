...
@section('content')
    <section class="content">
        <form method="POST" action="{{ route('admin.templates.loai-khach-hang.store') }}" class="form-create" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
                        <label for="nv_vanphong">Loại đương sự: (<span class="text-danger qksao">*</span>)</label>
                        <select id="loai_khach_hang_id" class="form-control select2" name="loai_khach_hang_id">
                            @foreach($loaiKhachHangList as $key => $loaiKhachHang)
                                <option value="{{ $loaiKhachHang->k_id }}">{{ $loaiKhachHang->k_nhan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
                        <label for="nv_vanphong">Template: (<span class="text-danger qksao">*</span>)</label>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            {{--$("#loai_khach_hang_id").val('{{ $loaiKhachHangList[0]->k_id }}').trigger('change');--}}
        });
    </script>
@stop
